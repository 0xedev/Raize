...
const vestingAddress =
  (import.meta.env.VITE_VESTING_CONTRACT_ADDRESS as Address) ||
  ("0x1234567890123456789012345678901234567890" as Address); // Fallback address, should be replaced with actual address
...
const CreatorPresaleCard: React.FC<CreatorPresaleCardProps> = ({
  ...
  const options =
    optionsResult?.status === "success"
      ...
      : undefined;
  currentPresaleOptionsForTimestampContext = {
    state:
      stateCallResult?.status === "success"
        ? stateCallResult.result
        : undefined,
  };
  ...
  const canWithdraw =
    ...
    ownerBalance > 0n; // Updated: Withdraw only possible in State 2 or 3 AND if ownerBalance > 0
      ...
      if (
        ...
        ((typeof originalErrorMessage === "string" &&
          ...
          (err.cause &&
            ...
            (err.cause as any).code === 4001)) // Wrapped EIP-1193
      ) {
...
const ContributedPresaleCard: React.FC<ContributedPresaleCardProps> = ({
  ...
  const options =
    optionsResult?.status === "success"
      ...
      : undefined;
  currentPresaleOptionsForTimestampContext = {
    state: stateResult?.status === "success" ? stateResult.result : undefined,
  };
  const state =
  ...
  const canClaim =
    ...
    (userClaimedAmountRaw !== undefined
      ...
      : true); // Updated: Claim only possible in State 3 (Finalized/Success)
...
const UserProfilePage = () => {
  ...
  const [isLoadingVestingSchedules, setIsLoadingVestingSchedules] =
    useState<boolean>(false);
  const [vestingClaimError, setVestingClaimError] = useState<string | null>(
    null
  );
  const [vestingClaimPresaleAddress, setVestingClaimPresaleAddress] = useState<
    string | null
  >(null);
  const [isVestingClaimPending, setIsVestingClaimPending] =
    useState<boolean>(false);
  const [isVestingClaimConfirming, setIsVestingClaimConfirming] =
    useState<boolean>(false);
  ...
  const loadVestingSchedules = async () => {
    ...
    try {
      ...
      const vestingScheduleContracts = contributedPresalesAddresses.map(
        (presaleAddress) => ({
      ...
      const claimableAmountContracts = contributedPresalesAddresses.map(
        (presaleAddress) => ({
          address: actualVestingAddress,
          abi: vestingAbi,
      ...
      const tokenAddressContracts = contributedPresalesAddresses.map(
        (presaleAddress) => ({
          address: presaleAddress,
          abi: presaleAbi,
          functionName: "token",
      ...
      // Batch read all data
      const [scheduleResults, claimableResults, tokenAddressResults] =
        await Promise.all([
      ...
      const processedSchedules = scheduleResults
        .map((result: { status: string; result: any }, index: number) => {
          if (result.status !== "success" || !result.result) return null;

          const schedule = result.result;
          const claimable =
            claimableResults[index]?.status === "success"
              ? claimableResults[index].result
              : 0n;
          const tokenAddress =
            tokenAddressResults[index]?.status === "success"
              ? tokenAddressResults[index].result
              : undefined;
          ...
          return {
            ...
            endTime:
              BigInt(schedule.start || 0) + BigInt(schedule.duration || 0),
      ...
      const tokenInfoPromises = processedSchedules.map(
        async (schedule: { tokenAddress: any }) => {
          ...
          try {
            ...
            return {
              ...schedule,
              tokenSymbol:
                symbolResult.status === "success" ? symbolResult.result : "TKN",
              tokenDecimals:
      ...
      <Tabs defaultValue="created">
        <div className="flex flex-col sm:flex-row justify-between items-center gap-2 mb-4">
          <TabsList className="grid w-full grid-cols-1 gap-1 sm:flex sm:w-auto sm:flex-wrap sm:gap-0">
            <TabsTrigger value="created" className="w-full sm:w-auto">
              My Created ({createdPresalesData?.length || 0})
            </TabsTrigger>
            <TabsTrigger value="contributed" className="w-full sm:w-auto">
              Contributed To ({contributedPresalesAddresses?.length || 0})
            </TabsTrigger>
            <TabsTrigger value="vesting" className="w-full sm:w-auto">
              My Vesting
            </TabsTrigger>
            <TabsTrigger value="history" className="w-full sm:w-auto">
              History
            </TabsTrigger>
          </TabsList>
          <Button
        ...
        <TabsContent value="created">
          ...
          {errorAllPresales && (
            <Alert variant="destructive">
              <AlertCircle className="h-4 w-4" />
              <AlertDescription>
                Error loading all presales list:{" "}
                {ensureString(errorAllPresales.message)}
              </AlertDescription>
            </Alert>
          )}
          {isErrorCreatedHook && (
            <Alert variant="destructive">
              <AlertCircle className="h-4 w-4" />
              <AlertDescription>
                Error loading created presales:{" "}
                {ensureString(errorCreatedHook?.message)}
              </AlertDescription>
            </Alert>
          )}
          {!isLoadingCreated &&
        ...
        <TabsContent value="contributed">
          ...
          {isErrorContributedAddressesHook && (
            <Alert variant="destructive">
              <AlertCircle className="h-4 w-4" />
              <AlertDescription>
                Error loading list of contributed presales:{" "}
                {ensureString(errorContributedAddressesHook?.message)}
              </AlertDescription>
            </Alert>
          )}
          {!isLoadingContributedAddresses &&
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription className="text-xs">
                Tokens being gradually unlocked from presales you participated
                in.
              </CardDescription>
            </CardHeader>
            <CardContent>
                      ...
                      <CardContent className="space-y-3">
                        <div className="w-full bg-secondary rounded-full h-2.5 mt-2">
                          <div
                            ...
                            style={{
                              width: `${Math.min(
                                100,
                                Number(schedule.progressPercentage)
                              )}%`,
                            }}
                          ></div>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2 mt-2">
                          <Button
                            ...
                            disabled={
                              schedule.claimableAmount <= 0n ||
                              isVestingClaimPending ||
                              isVestingClaimConfirming
                            }
                          >
                            Claim
                          </Button>
                        ...
                        {vestingClaimError &&
                          vestingClaimPresaleAddress ===
                            schedule.presaleAddress && (
                            <Alert variant="destructive" className="mt-2">
                              <AlertCircle className="h-4 w-4" />
                              <AlertDescription>
                                {ensureString(vestingClaimError)}
                              </AlertDescription>
                            </Alert>
                          )}

                        {(isVestingClaimPending || isVestingClaimConfirming) &&
                          vestingClaimPresaleAddress ===
                            schedule.presaleAddress && (
                            <Alert variant="default" className="mt-2">
                              <Info className="h-4 w-4" />
                              <AlertDescription>
                                {isVestingClaimConfirming
                                  ...
                                  : "Processing claim..."}
                                Tx:{" "}
                                {shortenAddress(vestingClaimHash || undefined)}
                              </AlertDescription>
                            </Alert>
                          )}
                      </CardContent>
...
function writeContractAsync(_arg0: {
  address: `0x${string}`;
  abi: Abi;
  functionName: string;
  args: string[];
